...
| Pattern | Main thread | How the response returns |
| --- | --- | --- |
| `allOf().join()` | waits at join, then resumes | it writes the response itself |
| return `CompletableFuture` | freed immediately | socket stays open; the completing pool thread writes it |
